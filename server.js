...
// CACHES EM MEMÓRIA RAM
const imageCache = new Map();
const indiceBuscaRapida = new Map(); // OTIMIZAÇÃO: Busca O(1) sem percorrer listas
...
// OTIMIZAÇÃO: Indexa os nomes dos jogadores na subida do servidor
function indexarJogadores() {
  Object.keys(BANCO_DE_CARTAS).forEach(chave => {
    const nomeLimpo = removerAcentos(chave);
    const nomeSemNumero = nomeLimpo.replace(/\s+\d+$/, '').trim();
    
    // Mapeia tanto a chave completa quanto a chave sem número para a chave original
    indiceBuscaRapida.set(nomeLimpo, chave);
    indiceBuscaRapida.set(nomeSemNumero, chave);
  });
}

// OTIMIZAÇÃO: Consulta instantânea via Hash Map
function encontrarChaveJogador(termoBusca) {
  const buscaLimpa = removerAcentos(termoBusca);
  if (!buscaLimpa) return null;

  // Busca rápida O(1)
  let achado = indiceBuscaRapida.get(buscaLimpa);
  if (achado) return achado;

  const buscaSemNumero = buscaLimpa.replace(/\s+\d+$/, '').trim();
  achado = indiceBuscaRapida.get(buscaSemNumero);
  if (achado) return achado;

  // Fallback caso a busca seja apenas parcial
  const chaves = Object.keys(BANCO_DE_CARTAS);
  return chaves.find(chave => {
    const nomeBancoLimpo = removerAcentos(chave).replace(/\s+\d+$/, '').trim();
...
async function preCarregarImagens() {
  ...
  console.log("⚡ Imagens e índices armazenados na RAM!");
}

// POSIÇÕES CONSTANTES (Evita recriar o objeto a cada request do /gerar-campo)
const POSICOES = {
  gr:  { x: 400, y: 705 },
  le:  { x: 100, y: 580 },
  dc1: { x: 270, y: 565 },
...
// -------------------------------------------------------------
// ROTA 1: GERAR IMAGEM DO CAMPO (800x800)
// -------------------------------------------------------------
app.get('/gerar-campo', (req, res) => {
  try {
    ...
    // OTIMIZAÇÃO: Cache HTTP no navegador/cliente
