...
// -------------------------------------------------------------
// ROTA 6: GERAR IMAGEM DO MULTISELL (1 A 20 JOGADORES)
// -------------------------------------------------------------
app.get('/gerar-multisell', async (req, res) => {
  try {
    ...
    // Separa a lista enviada por vírgula e remove vazios
    ...
    // Configuração do Grid Dinâmico (máximo 5 por linha)
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (urlCarta) {
        try {
          ...
          console.error(`Erro ao carregar carta ${listaCartas[i]}:`, err.message);
        }
      }
    }
